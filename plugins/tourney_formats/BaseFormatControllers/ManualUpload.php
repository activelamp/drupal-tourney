...
class ManualUploadController extends TourneyController {
  ...
  public function saveMatches() {
    ...
    foreach ($matches as $mid => $match) {
      ...
      // Add contestants to the matches right now.
      // Load first contestant.
      $elements = explode('.', $this->uploadSchema['contestants'][$match['seeds'][1]]);
      $contestant1 = entity_load($elements[0], array($elements[1]));
      if (empty($contestant1)) {
        throw new Exception("Entity not found: type: {$elements[0]} id: {$elements[1]}");
      }
      else {
        $contestant1 = $contestant1[$elements[1]];
      }
      // Load second contestant.
      $elements = explode('.', $this->uploadSchema['contestants'][$match['seeds'][2]]);
      $contestant2 = entity_load($elements[0], array($elements[1]));
      if (empty($contestant2)) {
        throw new Exception("Entity not found: type: {$elements[0]} id: {$elements[1]}");
      }
      else {
        $contestant2 = $contestant2[$elements[1]];
      }
      $new_match->addContestant($contestant1, 1)->save();
      $new_match->addContestant($contestant2, 2)->save();
